...
public class BookingServiceImpl implements BookingService {
	...
	public ResponseVO<OTPDto> validOtpAndstartRide(String otp) {
		...
		// Booking booking =
		// bookingRepository.findBookingsByUserAndStatus(userDriver.get().getUserId(),
		// "BOOKED");
		Optional<Driver> driver=driverRepository.findById(userDriver.get().getUserId());
		 List<Booking> bookings = bookingRepository.findByDriverAndStatus(driver.get(), "BOOKED");

		//List<Booking> bookings = bookingRepository.findByUserIdAndStatus(userDriver.get().getUserId(), "BOOKED");

		//System.out.println(bookings.toString());

		//Booking booking = bookingRepository.findByUserAndStatus(userDriver.get().getUserId(), "BOOKED");
